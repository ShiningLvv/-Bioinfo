...
BAM::BAM(int len, char *fileName)
{
    ...
    {
        fprintf(stderr, "[DelFeature_bam] ERROR: Could not open %s.\n", fileName);
        exit(1);
    }
    ...
    if (!open(index, bai1) && !open(index, bai2)) {
        fprintf(stderr, "[DelFeature_bam] ERROR: Could not open BAM index file.\n");
        exit(1);
    }
...
bool BAM::readRecord(char *chrName, int pos, int posEnd)
{
    ...
    //fprintf(stderr, "[DelFeature_bam] %s: %d-%d is reading...", chrName, pos, posEnd);
    bool hasAlignments;
...
double BAM::getDepth(char *chrName, int from, int to)
{
    ...
    if(from > to)
    {
        fprintf(stderr, "[DelFeature_bam]    getDepth %s:%d-%d error\n", chrName, from, to);
        exit(1);
    }
    int regionLen = 0;
    int depth;
